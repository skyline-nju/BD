...
template <typename T>  // Int-like type
...
void unpack_ghost_par(const double* buf, int buf_size,
  ...
  for (int buf_pos = 0; buf_pos < buf_size; ) {
...
public:
  ...
  Communicator_2(const TDomain& dm, const TGrid& grid, double rho0=1., double amplification=5.);

  ~Communicator_2() { for (auto i = 0; i < 4; i++) delete buf_[i]; }
