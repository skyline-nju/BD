...
int main(int argc, char* argv[]) {
...

  {
    ...
    //float* buf = new float[n_par_gl * 3];
    //int t_start = load_last_frame(prefix, buf);
    ///*
    char file_info[200];
    ...
    //  dm.ini_rand(p_arr, n_par_gl, bc);
    //} else {
    //  dm.ini_from_file(file_in, p_arr, n_par_gl);
    //}

    dm.set_buf(r_cut, 10);

    //XyzExporter_2 xy_outer(prefix, 0, n_step, 10000, gl_l, MPI_COMM_WORLD);
    SnapExporter_2 snap_outer(prefix, 0, n_step, 10000, file_info, MPI_COMM_WORLD);
    LogExporter log_outer(prefix, 0, n_step, 50000, n_par_gl, MPI_COMM_WORLD);
